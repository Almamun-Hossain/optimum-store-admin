...
const ProductTable = ({ products, onEdit, onDelete, currentPage, setCurrentPage, totalItems, itemsPerPage, setItemsPerPage }) => {
    ...
    return (
        <div className="border shadow-md px-3 py-2 rounded-lg">
            <div className="flex justify-between mb-4">
                <div>
                    <span>Show: </span>
                    <select className="form-select" onChange={(e) => setItemsPerPage(e.target.value)}>
                ...
                <tbody>
                    {products.map(product => (
                        <React.Fragment key={product.id}>
                            <tr onClick={() => toggleRow(product.id)} className={`cursor-pointer hover:bg-gray-100 ${expandedRows[product.id] ? 'bg-violet-400 text-white hover:text-gray-700' : ''}`}>
                                <td className="p-2">
                                    <input type="checkbox" />
                                </td>
                                ...
                                <td className="p-2">${product.variants[0]?.basePrice}</td>
                                <td className="p-2">
                                    <button className={`text-gray-500 hover:text-gray-700 `} onClick={() => onEdit(product)}>
                                        <FiEdit3 />
                                    </button>
                                    <button className={`text-gray-500 hover:text-gray-700 ml-2 `} onClick={() => onDelete(product.id)}>
                                        <FaRegTrashCan />
                                    </button>
                                </td>
                            </tr>
                                            ...
                                            <tbody>
                                                {product.variants.map(variant => (
                                                    <tr key={variant.id}>
                                                        ...
                                                        <td className="p-2">{variant.attributes.map(attr => attr.attributeValue).join(", ")}</td>
                                                        <td className="p-2">${variant.basePrice}</td>
                                                        <td className="p-2">${variant.salePrice || "N/A"}</td>
                                                        <td className="p-2">
